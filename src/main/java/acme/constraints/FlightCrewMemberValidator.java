...
public class FlightCrewMemberValidator extends AbstractValidator<ValidFlightCrewMember, FlightCrewMember> {
    ...
    public boolean isValid(final FlightCrewMember flightCrewMember, final ConstraintValidatorContext context) {
        ...
        if (flightCrewMember.getEmployeeCode() == null || flightCrewMember.getEmployeeCode().isBlank() ||
            !flightCrewMember.getEmployeeCode().matches("^[A-Z]{2,3}\d{6}$")) {
            context.disableDefaultConstraintViolation();
        ...
        String nombre = transliterate(userAccount.getIdentity().getName());
        String[] apellidos = transliterate(userAccount.getIdentity().getSurname()).split(" ");
        ...
        if (!iniciales.equals(identifierInitials)) {
            ...
            context.buildConstraintViolationWithTemplate(
                "Identifier must start with initials of the user: Should be " + iniciales + " but is " + identifierInitials
